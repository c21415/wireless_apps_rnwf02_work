...
RNWF_RESULT_t RNWF_NET_SOCK_Write( RNWF_NET_SOCKET_t *socket, uint16_t length, uint8_t *input)  {
    ...
    {     
        result = RNWF_CMD_RSP_Send(NULL, NULL, RNWF_SOCK_WRITE_TCP, socket->sock_client, length, input);
        //if()
        {
            //result = RNWF_IF_RAW_WRITE(input, length);        
        }
    }
    else
    {
        if(RNWF_CMD_RSP_Send(NULL, NULL, RNWF_SOCK_WRITE_UDP, socket->sock_client, socket->sock_addr, socket->sock_port, length) == RNWF_PASS)        
        {
            result = RNWF_IF_RAW_WRITE(input, length);        
        }
...
RNWF_RESULT_t RNWF_NET_SOCK_Read( RNWF_NET_SOCKET_t *socket, uint16_t length, uint8_t *buffer, RNWF_SOCK_MODE_t read_mode)  {                
    RNWF_RESULT_t result;
    if(result = RNWF_CMD_RSP_Send(RNWF_SOCK_READ_RESP, buffer, RNWF_SOCK_READ, socket->sock_client, read_mode, length) == RNWF_PASS)
    {
        // parse the socket id and length and return the offset
        while(*buffer != '"')
        {
            result++;     
        }
